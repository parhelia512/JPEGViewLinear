...
static void ParseCommandLineForStartupFile(TCHAR *sCommandLine)
	{
	TCHAR CommandLinePrivate[64000];
	wsprintf(CommandLinePrivate,TEXT("%s"),sCommandLine);

	CString sStartupFile = CommandLinePrivate;
	if (sStartupFile.GetLength() == 0)
		{
		wsprintf(sCommandLine,TEXT(""));
		return;
		}
	...
	wsprintf(sCommandLine,TEXT("%s"), (LPCTSTR) sStartupFile);
	return;
	}
...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE /*hPrevInstance*/, LPTSTR lpstrCmdLine, int /*nCmdShow*/)
	...
	TCHAR StartupFile[64000];
	wsprintf(StartupFile,TEXT("%s"),lpstrCmdLine);
	ParseCommandLineForStartupFile(StartupFile);
	if(*StartupFile == '\0')
		{
	...
	// BLOCK: Run application
	{
		...
		dlgMain.SetStartupFile(StartupFile);
		try
			{
